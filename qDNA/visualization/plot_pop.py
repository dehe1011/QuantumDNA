...
def plot_pops_heatmap(me_solver, vmax_list=[1, 1, 1], heatmap_type="seaborn"):
    ...
    fig, ax = plt.subplots(1, num_particles, figsize=(6.4 * num_particles, 4.8), sharey=False)
    ...
    for i, particle in enumerate(me_solver.tb_ham.particles):
        ...
        if heatmap_type == "matplotlib":
            im = ax[i].imshow(particle_pop, cmap=cmaps[i], aspect="auto", vmax=vmax_list[i])
        ...
        ax[i].set_xticks(np.linspace(0, x_len, 7), np.round(np.linspace(0, me_solver.t_end, 7), 0))
        ax[i].set_yticks(np.arange(y_len) + 0.5, labels=me_solver.tb_ham.tb_sites_flattened)
...
def plot_pop_fourier(ax, tb_ham, init_state, end_state, times, t_unit):
    ...
    # plotting
    for particle in tb_ham.particles:
        ax.plot(times, pop_dict[particle], label=particle, color=COLORS_PARTICLES[particle])
