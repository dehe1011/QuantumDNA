...
class ME_Solver:
    ...
    def get_init_matrix(self):
        ...
        if self.tb_ham.description == "2P":
            ...
            if self.me_kwargs["deloc_init_state"]:
                ...
                init_states = [
                    get_eh_observable(tb_basis, "exciton", state, state) for state in tb_basis
                ]
                if self.tb_ham.relaxation:
                    init_states = [add_groundstate(init_state) for init_state in init_states]
    ...
    def get_result_particle(self, particle):
        ...
        if not vars(self)["result_" + particle]:
            ...
            reduced_dms = [
                get_reduced_dm(dm, particle, self.tb_model.tb_basis) for dm in self.result
            ]
    ...
    def get_pop(self):
        ...
        if not self.pop:
            ...
            if self.tb_ham.description == "2P":
                ...
                keys = [
                    self.tb_ham.particles[0] + "_" + tb_site for tb_site in self.tb_ham.tb_basis
                ]
                values = [
                    q.fock_dm(self.tb_ham.matrix_dim, i) for i in range(self.tb_ham.matrix_dim)
                ]
    ...
    def get_coh(self):
        ...
        if not self.coh:
            ...
            if self.tb_ham.description == "1P":
                ...
                values = [
                    q.basis(self.tb_ham.matrix_dim, i) * q.basis(self.tb_ham.matrix_dim, j).dag()
            ...
            for particle in self.tb_ham.particles:
                ...
                for tb_site1, tb_site2 in permutations(self.tb_ham.tb_basis, 2):
                    value = 0
                    if self.qutip_version == "5":
                        value = result.e_data[particle + "_" + tb_site1 + "_" + tb_site2]
                    if self.qutip_version == "4":
                        value = result.expect[particle + "_" + tb_site1 + "_" + tb_site2]
