    ...
    def get_matrix(self):
        ...
        if self.description == "1P":
            ...
            if self.particles == ["electron"]:
                matrix = tb_ham_1P(self.tb_model, self.tb_params_electron, self.tb_basis_sites_dict)

            # generate the Hamiltonian matrix for the hole
            if self.particles == ["hole"]:
                matrix = tb_ham_1P(self.tb_model, self.tb_params_hole, self.tb_basis_sites_dict)

            # generate the Hamiltonian matrix for the exciton
            if self.particles == ["exciton"]:
                matrix = tb_ham_1P(self.tb_model, self.tb_params_exciton, self.tb_basis_sites_dict)
        ...
        for particle in self.particles:
            if self.description == "2P":
                ...
                end_states_idx = [
                    ...
                    for end_state in get_particle_eh_states(particle, end_state, self.tb_basis)
                ]
                ...
                if "amplitude" in quantities:
                    amplitudes_dict[particle] = list(
                        chain.from_iterable(
                            [calc_amplitudes(eigs, init_state_idx, es) for es in end_states_idx]
                        )
                    )
                ...
                if "average_pop" in quantities:
                    average_pop_dict[particle] = np.sum(
                        [calc_average_pop(eigs, init_state_idx, es) for es in end_states_idx]
                    )
            if self.description == "1P":
                # calculate amplitude
                if "amplitude" in quantities:
    ...
    def get_backbone_pop(self, init_state):
        ...
        for particle in self.particles:
            for tb_site in backbone_sites:
                backbone_pop[particle] += self.get_average_pop(init_state, tb_site)[particle]
        return backbone_pop
