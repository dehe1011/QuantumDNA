...
def find_xyz_files(directory):
    ...
    try:
        ...
        xyz_files = [os.path.splitext(file)[0] for file in files if file.endswith(".xyz")]
...
def load_xyz(filename, directory=os.getcwd()):
    ...
    xyz_data = pd.DataFrame([line.split() for line in lines[2:]], columns=["Atom", "X", "Y", "Z"])
...
def convert_json_to_xyz(filename, directory):
    ...
    for i in range(num_atoms):
        element_symbol = atomic_symbols.get(elements[i], "X")  # Default to 'X' if not found
        x = x_coords[i]
        y = y_coords[i]
        z = z_coords[i]
