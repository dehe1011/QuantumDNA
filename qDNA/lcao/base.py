...
class Orbital:  # pylint: disable=too-few-public-methods
    ...
    def __repr__(self):
        return f"Orbital({self.coordinates}, {self.atom_identifier}, {self.orbital_type})"
...
class Base:
    ...
    def __init__(self, xyz_identifier, xyz_data):
        ...
        self.orbital_coordinates = np.array([orbital.coordinates for orbital in self.orbitals])
        ...
        self.relative_orbital_coordinates = np.array(
            [coordinates - self.center_of_mass for coordinates in self.orbital_coordinates]
        )
        ...
        self.oscillator_strength = self.calc_oscillator_strength(self.dipole_moment)  # no unit
    ...
    def calc_oscillator_strength(self, dipole_moment):
        ...
        return (
            2 / 3 * c.m_e / (c.e**2 * c.hbar**2) * self.E_exc * c.e * (dipole_moment * 1e-10) ** 2
        )
    ...
    def _get_atom_coordinates(self):
        ...
        for atom_idx in range(self.num_atoms):
            ...
            atom_coordinates[atom_idx] = np.array([xyz_entry["X"], xyz_entry["Y"], xyz_entry["Z"]])
    ...
    def _get_orbital_distance_matrix(self):
        ...
        for i, j in combinations(range(self.num_orbitals), r=2):
            value = np.linalg.norm(self.orbital_coordinates[i] - self.orbital_coordinates[j])
    ...
    def _calc_H(self):
        ...
        # orbital overlap
        for i, j in combinations(range(self.num_orbitals), r=2):
            value = calc_orbital_overlap(self.orbitals[i], self.orbitals[j], connection="intrabase")
            H[i, j] = value
            H[j, i] = value
