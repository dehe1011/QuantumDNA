...
@pytest.mark.parametrize("omega, cutoff_freq, reorg_energy, expected", [
    (0, 1.0, 1.0, 0),
...
@pytest.mark.parametrize("omega, cutoff_freq, reorg_energy, exponent, expected", [
    (0, 1.0, 1.0, 1.0, 0),
...
@pytest.mark.parametrize("omega, temperature, expected", [
    (0, 300, np.inf),
...
@pytest.mark.parametrize("omega, deph_rate, cutoff_freq, reorg_energy, temperature, spectral_density, exponent, expected", [
    (0, None, 1.0, 1.0, 300, "debye", None, (4 * 1.0 * c.k * 300) / (c.hbar * 1.0 * 1e12)),
    (1.0, 0.5, 1.0, 1.0, 300, "debye", None, (2 * (2.0 * 1.0 * 1.0 / (1.0**2 + 1.0**2)) * (1 + (1.0 / (np.exp(c.hbar * 1.0 * 1e12 / (c.k * 300)) - 1))))),
])
def test_rate_constant_redfield(omega, deph_rate, cutoff_freq, reorg_energy, temperature, spectral_density, exponent, expected):
    result = rate_constant_redfield(omega, deph_rate, cutoff_freq, reorg_energy, temperature, spectral_density, exponent)
    assert np.isclose(result, expected)

@pytest.mark.parametrize("cutoff_freq, reorg_energy, temperature, expected", [
