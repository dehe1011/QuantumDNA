...
from DNA import TB_Ham, DNA_Seq, TB_Model, Lindblad_Diss, ME_Solver
from user_interface import CustomWindow, InitialFrame, ConfigFrame, OptionsFrame, ScrollableConsoleFrame, PlotOptionsFrame, PlottingWindow
...
class qDNA_App(ctk.CTk):
    ...
    def get_init_kwargs(self):
        ...
        self.init_kwargs = {"upper_strand": self.upper_strand,
    ...
    def get_options_kwargs(self):
        ...
        self.options_kwargs = dict(**self.ham_kwargs, **self.diss_kwargs, **self.me_kwargs)
    ...
    def get_plot_options_kwargs(self):
        self.plot_option = {"plot_option": self.plot_options_frame.plotting_options_tab.get()}
        self.pop_kwargs = self.plot_options_frame.plotting_options_tab.pop_frame.get_pop_kwargs() 
        self.coh_kwargs = self.plot_options_frame.plotting_options_tab.coh_frame.get_coh_kwargs() 
        self.fourier_kwargs = self.plot_options_frame.plotting_options_tab.fourier_frame.get_fourier_kwargs() 
        self.plot_options_kwargs = dict(**self.plot_option, **self.pop_kwargs, **self.coh_kwargs, **self.fourier_kwargs)
    ...
    def press_first_confirm(self):
        ...
        # since the TB basis is known we can configure some widgets
        self.options_frame.options_tab.dynamics_frame.me_init_e_state_combo.configure(values=self.tb_model.tb_basis)
        self.options_frame.options_tab.dynamics_frame.me_init_h_state_combo.configure(values=self.tb_model.tb_basis)
        self.plot_options_frame.plotting_options_tab.pop_frame.tb_site_combo.configure(values=self.tb_model.tb_basis)
        self.plot_options_frame.plotting_options_tab.fourier_frame.tb_site_combo.configure(values=self.tb_model.tb_basis)
        self.plot_options_frame.plotting_options_tab.fourier_frame.init_e_site_combo.configure(values=self.tb_model.tb_basis)
        self.plot_options_frame.plotting_options_tab.fourier_frame.init_h_site_combo.configure(values=self.tb_model.tb_basis)
    ...
    def press_second_confirm(self):
        ...
        # TODO: configure some widgets using the chosen inputs
